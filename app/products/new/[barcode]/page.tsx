...
export default function ProductReviewForm({
  ...
  const hasRetrievedDetails =
    lookupStatus === "success" || lookupStatus === "partial";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (result.data?.selling_price)
        setPrice(String(result.data.selling_price));
      if (result.data?.stock_quantity)
        setStock(String(result.data.stock_quantity));
  ...
  async function handleSave() {
    if (!name.trim() || !price || !stock) {
      setSaveError(
        "Enter a product name, selling price, and stock quantity before saving.",
      );
      return;
    }
      ...
      <div className="w-full max-w-2xl bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="p-6 space-y-8">
          {lookupStatus === "success" ? (
              ...
              <div className="flex-1">
                ...
                {lookupSource && (
                  <span className="text-xs text-green-600 mt-1 block">
                    Source:{" "}
                    {lookupSource === "open_facts"
                      ? "📚 Public product database"
                      : lookupSource === "internet_search"
                        ? "🌐 Internet Search"
            ...
            <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-lg">
              This barcode is already in your inventory and cannot be added
              again.
            </div>
          ) : lookupStatus === "error" ? (
            <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg">
              We could not look up this barcode right now. You can still enter
              the product details manually.
            </div>
          ) : (
